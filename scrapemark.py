...
def scrape(pattern, html=None, url=None, get=None, post=None, headers=None, cookie_jar=None, encoding=None):
	if type(pattern) == str:
		pattern = compile(pattern)
	return pattern.scrape(html, url, get, post, headers, cookie_jar, encoding)
	
def compile(pattern):
	return _Pattern(_compile(pattern, True))
	
def fetch_html(url, get=None, post=None, headers=None, cookie_jar=None, encoding=None):
	if get:
		if type(get) == str:
...
class _Pattern:
	...
	def scrape(self, html=None, url=None, get=None, post=None, headers=None, cookie_jar=None, encoding=None):
		if cookie_jar == None:
			cookie_jar = cookielib.CookieJar()
		if html == None:
			html = fetch_html(url, get, post, headers, cookie_jar, encoding)
		captures = {}
